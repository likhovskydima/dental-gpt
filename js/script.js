...
const motionQuery = typeof window !== 'undefined' && 'matchMedia' in window ? window.matchMedia('(prefers-reduced-motion: reduce)') : null;
const floatingCards = document.querySelectorAll('.floating-card');
const heroOrbs = document.querySelectorAll('.hero__orb');
let floatAnimationFrame;

const animateHeroElements = (time = 0) => {
  const elapsed = time * 0.001;

  floatingCards.forEach((card, index) => {
    const offset = Math.sin(elapsed * 1.4 + index) * 6;
    card.style.transform = `translate3d(0, ${offset}px, 0)`;
  });

  heroOrbs.forEach((orb, index) => {
    const offset = Math.cos(elapsed * 0.8 + index) * 12;
    orb.style.transform = `translate3d(0, ${offset}px, 0)`;
  });

  floatAnimationFrame = requestAnimationFrame(animateHeroElements);
};
  ...
  if (!priceDisplay || !durationDisplay || !complexityDisplay || !breakdownList || !slider || !sliderBubble) {
    ...
    const pricingMatrix = {
      ...
      aesthetics: {
        ...
        breakdown: ['Digital Smile Design', 'Підготовка емалі', 'Виготовлення індивідуальних вінірів']
      },
      orthodontics: {
        label: 'Ортодонтія',
        base: 32000,
        ...
        breakdown: ['3D-план руху зубів', 'Набір елайнерів/брекетів', 'Щомісячні контрольні візити']
      },
      surgery: {
    ...
    const updateSliderBubble = () => {
      ...
      const bubbleWidth = sliderBubble.offsetWidth || 0;
      const offset = sliderRect.width * ratio;
      const clampedOffset = Math.min(Math.max(offset, bubbleWidth / 2), sliderRect.width - bubbleWidth / 2);
      sliderBubble.textContent = complexityLabels[value];
      sliderBubble.style.left = `${clampedOffset}px`;
      sliderBubble.style.transform = 'translate(-50%, -120%)';
    };
...
function initToothScene() {
  ...
  scene.fog = new THREE.FogExp2(0xe8f1ff, 0.12);
